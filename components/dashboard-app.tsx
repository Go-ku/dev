...
const currency = (value: number) => new Intl.NumberFormat("en-ZM", { style: "currency", currency: "ZMW" }).format(value);
  ...
  const leaseColumns = useMemo<ColumnDef<Lease>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => <span>{currency(getValue<number>())}</span>
      },
      {
        header: "Next Review",
        accessorKey: "nextReview",
        cell: ({ getValue }) => format(new Date(getValue<string>()), "d MMM")
      },
      {
        header: "Lease Ends",
        accessorKey: "expiresOn",
        cell: ({ getValue }) => format(new Date(getValue<string>()), "d MMM yyyy")
      },
      {
        header: "Status",
        accessorKey: "status",
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-base text-slate-600">
              Track rent, leases, maintenance and landlord communications from a single
              minimalist workspace. Optimised for mobile teams but delightful on desktop too.
            </p>
          </div>
          <div className="grid gap-2 text-sm text-slate-600">
            <p>
              <span className="font-semibold text-slate-900">System admin:</span> full access to
              everything.
            </p>
            <p>
              <span className="font-semibold text-slate-900">Managers:</span> register tenants,
              log payments, create invoices & reminders.
            </p>
            <p>
              <span className="font-semibold text-slate-900">Landlords:</span> read-only view of
              leases, arrears and maintenance.
            </p>
            <p>
              <span className="font-semibold text-slate-900">Maintenance crew:</span> live queue,
              priority tags and job notes.
            </p>
            <p>
              <span className="font-semibold text-slate-900">Tenants:</span> mobile-friendly
              payment receipts, lease info and service desk.
            </p>
          </div>
        </div>
      </section>
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Managers can capture payments on behalf of tenants, auto-tag landlord share and send
                branded receipts.
              </CardDescription>
            </div>
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              reminderMutation.mutate({
                ...
                channel: reminderForm.channel as Reminder["channel"]
              });
            }}
          >
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => setReminderForm((prev) => ({ ...prev, tenant: e.target.value }))}
              />
            </div>
            <div className="space-y-1">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <Table>
              <THead>{leaseTable.getHeaderGroups().map((headerGroup) => (
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>
                Log requests on behalf of tenants, auto-assign crew and keep everyone updated.
              </CardDescription>
            </div>
          ...
          <form
            ...
            onSubmit={(event) => {
              ...
              maintenanceMutation.mutate({
                ...
                priority: maintenanceForm.priority as MaintenanceTicket["priority"],
                status: maintenanceForm.status as MaintenanceTicket["status"]
              });
            }}
          >
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => setMaintenanceForm((prev) => ({ ...prev, property: e.target.value }))}
              />
            </div>
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => setMaintenanceForm((prev) => ({ ...prev, tenant: e.target.value }))}
              />
            </div>
            <div className="space-y-1">
